...
const CHECKIN_PREFIX = 'Daily check-in';
...
function streakEnding(days, end) {
...
export async function checkin(commerce, shop, options = {}) {
  const today = options.date ?? new Date().toISOString().slice(0, 10);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(today)) {
    throw new Error('--date must be YYYY-MM-DD');
  }
  const customer = await commerce.customers.getByEmail(shop.customer.email);
  if (!customer) throw new Error(`No customer ${shop.customer.email} in this database. Launch the shop first.`);
  const program = (await commerce.loyalty.listPrograms()).find((p) => p.name === shop.loyalty.program);
  if (!program) throw new Error(`No loyalty program ${shop.loyalty.program} in this database. Launch the shop first.`);
  const account = await commerce.loyalty.getAccountByCustomer(customer.id, program.id);
  if (!account) throw new Error(`Customer is not enrolled in ${shop.loyalty.program}. Launch the shop first.`);

  const txs = await commerce.loyalty.getTransactions(account.id, 1000);
  const days = [...new Set(
    txs
      .map((t) => (t.transactionType === 'earn' ? String(t.description ?? '').match(/^Daily check-in (\d{4}-\d{2}-\d{2})/) : null))
      .filter(Boolean)
      .map((m) => m[1]),
  )];
  if (days.includes(today)) {
    const balance = (await commerce.loyalty.getAccount(account.id)).pointsBalance;
    return { alreadyCheckedIn: true, streak: streakEnding(days, today), awarded: 0, balance, date: today };
  }
  const streak = 1 + streakEnding(days, shiftDay(today, -1));
  const settings = shop.loyalty.checkin ?? { points: 10, streakBonus: 0 };
  const awarded = settings.points + (streak - 1) * (settings.streakBonus ?? 0);
  await commerce.loyalty.adjustPoints({
    accountId: account.id, points: awarded, transactionType: 'earn',
    description: `${CHECKIN_PREFIX} ${today} (day ${streak})`,
  });
  const balance = (await commerce.loyalty.getAccount(account.id)).pointsBalance;
  return { alreadyCheckedIn: false, streak, awarded, balance, date: today };
}
